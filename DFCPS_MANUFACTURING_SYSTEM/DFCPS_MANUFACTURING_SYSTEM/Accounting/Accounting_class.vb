...
Public Class Accounting_class
    ...
    Public Sub PRINT_VOUCHER_SLIP()
        Try
            ...
            For Each row As DataRow In dt.Rows
                ds.Tables("VOUCHERTABLE").Rows.Add(row(0), row(1), row(2), row(3), row(4), row(5), row(6), row(7), row(8), row(9))
            Next
    ...
    Public Sub PRINT_GENERAL_LEDGER()
        Try
            ...
            da.Fill(dt)
            For Each row As DataRow In dt.Rows
                ds.Tables("GLTABLE").Rows.Add(DFROM, DTO, row(0), row(1), row(2), row(3), row(4), row(5), row(6), row(7), row(8), row(9))
            Next
    ...
    Public Sub GET_CASH_REQUISISTION_FORPRINT()
        Try
            ...
            da.Fill(dt)
            For Each row As DataRow In dt.Rows
                ds.Tables("CASHREQTABLE").Rows.Add(row(0), CDate(row(1)).ToString("MMM dd, yyyy"), row(2), row(3), row(4), row(5), row(6), row(7), row(8), row(9))
            Next
        ...
        Try
            ...
            For Each row As DataRow In dt.Rows
                ...
                ds.Tables("CVTABLE").Rows.Add(row(0), CDate(row(1)).ToString("MMMM dd, yyyy"), row(2), row(3), row(4), row(5), row(6), row(7), row(8), row(9), row(10), ConvertNumberToENG(CDec(row(10))), row(11), _
                                              ref, row(13), row(14), row(15), row(16), MainForm.LBLNAME.Text)
            Next
            rptDoc = New CV_SLIP
